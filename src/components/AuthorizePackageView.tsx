        ...
        <div className="relative z-10 space-y-3">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-4 text-xs text-zinc-400">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span>Instant Cashback Bonus Guarantee</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="pt-4 border-t border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-xs text-zinc-400">
            {isPurchased ? (
              ...
              <span>Instant M-Pesa payment prompt upon activation.</span>
            )}
          </div>
          ...
          {isPurchased ? (
            ...
            <button
              ...
              className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-gradient-to-r from-blue-600 to-emerald-600 hover:from-blue-500 hover:to-emerald-500 text-white font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-blue-950/40 transition active:scale-[0.99]"
            >
              {isProcessing ? (
                <span className="flex items-center gap-2">
                  <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  Activating Package via M-Pesa...
                </span>
              ) : (
                <>
                  <Zap className="w-4 h-4" />
                  <span>Activate Authorize Package (KES {(pkg?.price || 0).toLocaleString()})</span>
                </>
              )}
            </button>
          )}
