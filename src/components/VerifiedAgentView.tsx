...
interface VerifiedAgentViewProps {
  currentUser: User;
  onActivateVerifiedAgent: () => void;
  onSwitchView: (view: AppView) => void;
}

export const VerifiedAgentView: React.FC<VerifiedAgentViewProps> = ({
  currentUser,
  onActivateVerifiedAgent,
  onSwitchView,
}) => {
  const [isProcessing, setIsProcessing] = useState(false);
  const pkg = PIPELINE_PACKAGES.verifiedAgent;
  const isPurchased = currentUser.isVerifiedAgentPurchased;
  ...
  return (
    <div className="space-y-6 pb-12 animate-fadeIn max-w-4xl mx-auto">
      {/* Banner */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-950 via-zinc-900 to-zinc-950 border border-amber-500/40 p-6 sm:p-8 shadow-xl">
        <div className="relative z-10 space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-300 text-xs font-semibold">
            ...
            <span>Executive Verified Agent Partner</span>
          </div>
          ...
          <div className="pt-2 flex flex-wrap items-center gap-4 text-xs text-zinc-400">
            <div className="flex items-center gap-1.5">
              ...
              <span>Full Tier 1 Partner Accreditation</span>
            </div>
            <div className="flex items-center gap-1.5">
        ...
        <div className="pt-4 border-t border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-xs text-zinc-400">
            {isPurchased ? (
              <span className="text-emerald-400 font-bold flex items-center gap-1.5">
                <Check className="w-4 h-4" /> Verified Agent Status Active!
              </span>
            ) : (
              <span>Instant payment notification to finalize accreditation.</span>
            )}
          </div>
          ...
          {isPurchased ? (
            ...
            <button
              ...
              className="w-full sm:w-auto px-8 py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition active:scale-[0.99] bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-black shadow-lg shadow-amber-950/40"
            >
              {isProcessing ? (
                ...
                <>
                  ...
                  <span>Activate Verified Agent (KES {(pkg?.price || 0).toLocaleString()})</span>
                </>
              )}
            </button>
          )}
