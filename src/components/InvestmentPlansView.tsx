        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-2 max-w-2xl">
            ...
            <p className="text-xs sm:text-sm text-zinc-300 leading-relaxed">
              Start growing your passive capital with packages beginning from <span className="text-emerald-400 font-bold">KES 1,500</span> and enjoy up to <span className="text-amber-400 font-bold">300% Monthly Yield</span>. All contracts are 100% principal insured under institutional escrow.
            </p>
          </div>
          ...
          <div className="bg-zinc-950/90 border border-zinc-800 rounded-2xl p-4 text-center shrink-0 w-full sm:w-auto space-y-1">
            ...
            <span className="text-[10px] text-zinc-400 block">
              Total Yield Harvested: <strong className="text-amber-300 font-mono">KES {(activeInvestments?.reduce((acc, c) => acc + (c.currentEarned || 0), 0) || 0).toLocaleString()}</strong>
            </span>
          </div>
        </div>
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className="w-full py-2.5 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-md shadow-emerald-950/40 transition active:scale-[0.99]"
              >
                {isProcessing ? (
                  ...
                  <>
                    <span>Invest KES {(plan.minDeposit || 0).toLocaleString()}</span>
      ...
      <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-6 space-y-4">
        ...
        {activeInvestments.length === 0 ? (
          <div className="p-8 text-center bg-zinc-950 rounded-xl border border-zinc-800">
            ...
            <p className="text-xs text-zinc-400">You have no active investment contracts. Choose a plan above to start earning 300% monthly return.</p>
          </div>
        ) : (
                ...
                >
                  <div className="space-y-2 max-w-md w-full">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-zinc-400">
                      <div>Principal: <strong className="text-zinc-200 font-mono">KES {(inv.amountInvested || 0).toLocaleString()}</strong></div>
                      <div>Total Target (400%): <strong className="text-emerald-400 font-mono">KES {(inv.expectedTotalPayout || 0).toLocaleString()}</strong></div>
                    </div>
                  ...
                  <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
                    <div className="text-right">
                      <span className="text-[10px] text-zinc-400 uppercase font-bold block">Current Earned Yield</span>
                      <span className="text-xl font-black font-mono text-emerald-400">
                        KES {(inv.currentEarned || 0).toLocaleString()}
                      </span>
                      <span className="text-[10px] text-zinc-500 block">Matures in 18 days</span>
                    </div>

                    <button
                      onClick={() => handleHarvest(inv.id)}
                      disabled={isHarvesting}
                      className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center gap-1.5 transition active:scale-[0.99] shadow-md shadow-emerald-950/40 shrink-0"
                    >
                      {isHarvesting ? (
